...
class Video2AudioConvertor < Logger::Application
...
Basic Command Line Usage:
  #{PROGRAM} --recursive <video file path> -base ~/Music
...
HELP
  ...
  #
  def argument_verify
    @log.debug { "option size is #{ARGV.size}" }
    case ARGV.size
    when 0
      @log.warn { "option nothing" }
      exit 0
    when 1..2
      @log.debug { "option at: 0. ; => #{ARGV[0]}" }
      @log.debug { "option at: 1. ; => #{ARGV[1]}" } if ARGV.size == 2
      ...
      @log.fatal { "#{PROGRAM}: invalid options. run `#{PROGRAM} --help' for assistance." }
      exit 0
    end
    ...
    if @options.has_key? :audio_basepath then
      ...
      @log.info { "no setting audio base path" }
    end

    if @options.has_key? :recursive then
      @search_recursive_p = true
      @log.info { "search recursive on" }
    else
      @search_recursive_p = false
      @log.info { "search recursive false" }
    end
    ...
    if @log.debug? then
      ...
      @log.debug { "input parent path: #{@input_parent_path}" }
      @log.debug { "output path: #{@output_path}" }
      @log.debug { "output parent path: #{@output_parent_path}" }
      @log.debug { "output base path: #{@output_base_path}" }
    end
  end
  ...
  def found_path(path, absent_creation = false)
    ...
    @log.debug { "original dir path: #{original_parent_path}" }
    if absent_creation and not File.exist? original_parent_path then
      @log.debug { "create dir #{original_parent_path}" }
      FileUtils.mkdir_p original_parent_path
    else
      unless File.exist? original_parent_path then
        @log.fatal { "#{PROGRAM}: cannot access to #{original_parent_path}: no such file or directory" }
        exit 0
      end
    end
  ...
  def ffmpeg_convert(infile, outfile, options = {})
    ...
    if File.exist? outfile then
      @log.debug { "#{File.ctime( outfile )} > #{File.ctime( infile )} # => #{File.ctime( outfile ) > File.ctime( infile )}" }
      return 1 if File.ctime( outfile ) > File.ctime( infile )
    end
    @log.debug { "ffmpeg -i '#{infile}' -y -vn -acodec #{options[:audio_codec]} -ab #{options[:audio_bitrates]} -ar #{options[:audio_sampling]} -ac #{options[:audio_channel]} '#{outfile}'" }
  ...
  def convert_many_files
    @log.debug { "#{@input_path} is directory." }
  ...
  def convert_recursive_many_files
    ...
    Find.find( @input_parent_path ) do |path|
      if File.directory? path then
        ...
        @log.debug { "temp parent path: #{temp_parent_path}" }

        unless temp_parent_path == output_parent_path then
          @log.debug { "parent path: #{temp_parent_path}" }
...
if $0 == __FILE__ then
  progn = Video2AudioConvertor.new
  status = progn.start
end
